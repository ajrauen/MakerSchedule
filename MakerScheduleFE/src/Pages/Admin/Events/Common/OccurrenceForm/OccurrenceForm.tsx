...
const OccurrenceForm = ({
  ...
  const { mutate: deleteOccurrenceMutation, isPending: isDeletePending } =
    useMutation({
  ...
  useEffect(() => {
    ...
    const eventId = selectedEventOccurrence?.meta?.isNew
      ? selectedEvent?.id
      : selectedEventOccurrence.eventId;
  ...
  const handleDeleteOccurrenceClick = () => {
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSave)}>
        <div className="p-4 space-y-2 bg-white rounded shadow flex flex-col gap-6">
          <FormSelect
            ...
            label={"Event"}
            disabled={!selectedEventOccurrence?.meta?.isNew}
            className={
              selectedEventOccurrence?.meta?.componentOrigin ===
              "occurrenceCalendar"
                ? ""
          ...
          <FormFooter
            showDelete={
              selectedEventOccurrence?.meta?.componentOrigin ===
                "occurrenceCalendar" && !selectedEventOccurrence.meta.isNew
            }
            onDelete={handleDeleteOccurrenceClick}
            onCancel={onCancel}
            areActionsDisabled={
      ...
      <ConfirmationDialog
        ...
        onConfirm={handleConfirmDeleteOccurrence}
